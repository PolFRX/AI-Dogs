...
def get_files(categories):
    ...
    length_training_data = math.floor(len(labels) * 0.95)
    ...
    number_elements_training = math.floor(len(labels) * 0.95)
    number_elements_validation = len(labels) - number_elements_training

    return labels_training, image_path_training, labels_validation, \
        image_path_validation, number_elements_training, number_elements_validation
...
def rename_directories_standford():
    """ Rename all the directories of the standford's dataset
    to take off ugly parts and thus make better categories for the model. """
...
def get_files_for_some_categories(categories):
    ...
    # Browse the "dataset" directory to get all paths for images and labels which are the directories names
    for dirs in dataset_dirs:
        if(dirs == "pug" or dirs == "English_setter" or dirs == "German_sheperd"
